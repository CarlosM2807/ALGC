    ...
    while(pila):
        ...
        if(actual not in visitados):                                    # SI ESTE NO HA SIDO VISITADO
            ...
            for i in range(len(V)):                                         # RECORRE EL ARRAY DE NODOS
                if(V[i] not in visitados and  (([int(actual[0])]),[int(V[i][0])]) in E):       # SI NO ESTA EN VISITADOS, Y ESTA EN E (ARRAY DE EJES)
...
visitados = DFS(V,listaAdyacencia,habitacionInicio)            # VISITADOS CONTIENE LAS HABITACIONES QUE RECORRE EL ALGORITMO
#visitados = BFS(V,listaAdyacencia,habitacionInicio)
